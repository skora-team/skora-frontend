...
import { LayoutDashboard, LogOut, Cpu, Sun, Moon } from 'lucide-react';
...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen transition-colors duration-300 bg-[var(--bg-main)] text-[var(--text-main)] relative">
      <aside className="w-72 bg-[var(--bg-sidebar)] border-r-4 border-[var(--border-color)] hidden md:flex flex-col relative z-20 transition-colors duration-300">
        ...
        <nav className="flex-1 p-6 space-y-6">
          ...
          <Link to="/" className="group flex items-center space-x-4 p-3 hover:bg-[var(--accent-glow)] border-l-4 border-transparent hover:border-[var(--accent)] transition-all">
        ...
        <div className="p-6 border-t-4 border-[var(--border-color)] space-y-4">
          <button onClick={toggleTheme} className="flex items-center justify-between w-full p-3 bg-[var(--bg-main)] border-2 border-[var(--border-color)] rounded hover:border-[var(--accent)] transition-all">
            <span className="text-xs font-bold text-[var(--text-muted)]">{theme === 'dark' ? 'LIGHT MODE' : 'DARK MODE'}</span>
            {theme === 'dark' ? <Sun size={18} className="text-yellow-400" /> : <Moon size={18} className="text-slate-600" />}
          </button>
          <button className="flex items-center space-x-3 text-red-500 hover:text-red-400 w-full p-2 transition-colors">
            <LogOut size={18} />
